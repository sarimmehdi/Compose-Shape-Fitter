...
import com.sarim.composeshapefittersampleapp.presentation.component.DEFAULT_STROKE_WIDTH

fun getDrawableShapeFromShape(shape: Shape) = when (shape) {
    ...
    Shape.OrientedRectangle -> ObbShape(Color.Blue, DEFAULT_STROKE_WIDTH, allSidesEqual = true, inPreviewMode = true)
    Shape.OrientedSquare -> ObbShape(Color.Blue, DEFAULT_STROKE_WIDTH, allSidesEqual = false, inPreviewMode = true)
...
fun generateDummyPoints(drawableShape: DrawableShape): List<Offset> {
    ...
    return when (drawableShape) {
        is CircleShape -> generateEllipsePoints(
            commonStartX + defaultSize,
            commonStartY + defaultSize,
            defaultSize,
            defaultSize,
        )
        is EllipseShape -> generateEllipsePoints(
            commonStartX + defaultSize,
            commonStartY + defaultSize,
            defaultSize * 1.5f,
            defaultSize,
        )
        is HexagonShape -> generatePolygonPoints(
            6, commonStartX, commonStartY
        )
        is ObbShape -> if (drawableShape.allSidesEqual) {
            listOf(
        ...
        is PentagonShape -> generatePolygonPoints(
            5, commonStartX, commonStartY
        )
        is RectangleShape -> listOf(
            Offset(commonStartX, commonStartY),
        ...
        is SkewedEllipseShape -> generateEllipsePoints(
            commonStartX + defaultSize,
            commonStartY + defaultSize,
            defaultSize,
            defaultSize * 0.7f,
...
//val drawingScreenDataExhaustiveSequence = Shape.entries.asSequence().flatMap { selectedShape ->
//    Exhaustive.boolean().values.asSequence().flatMap { isDragging ->
//        listOf(
...
//        ).asSequence().flatMap { drawableShape ->
//            Exhaustive.boolean().values.asSequence().flatMap { showSettingsDropDown ->
//                Exhaustive.boolean().values.asSequence().flatMap { showFingerTracedLines ->
//                    Exhaustive.boolean().values.asSequence().flatMap { showApproximatedShape ->
//                        DrawerValue.entries.asSequence().map { drawerValue ->
//                            DrawingScreenData(
