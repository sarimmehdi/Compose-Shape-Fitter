...
class DrawingScreenViewModelInitTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun testGetSettingsUseCase() = runTest(testDispatchers.testDispatcher) {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun testGetSelectedShapeUseCase() = runTest(testDispatchers.testDispatcher) {
    ...
    companion object {
        ...
        val successfulSettingsEmissions = Exhaustive.boolean().flatMap { showFingerTracedLines ->
        ...
        val drawingScreenUseCases = mockk<DrawingScreenUseCases>(relaxed = true) {
            every { getSettingsUseCase() } returns flow {
            ...
            every { getSelectedShapeUseCase() } returns flow {
                delay(
                    2 * DELAY_BETWEEN_EMISSIONS
                            + successfulSettingsEmissions.values.size * DELAY_BETWEEN_EMISSIONS
                )
